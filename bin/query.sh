...
for workload in $WORKLOAD_LIST; do
  LOAD_OPTS="-db site.ycsb.db.couchbase3.CouchbaseQuery -P $workload -threads $THREADCOUNT_LOAD -p recordcount=$RECORDCOUNT -s -load $EXTRAARGS"
  RUN_OPTS="-db site.ycsb.db.couchbase3.CouchbaseQuery -P $workload -threads $THREADCOUNT_RUN -p recordcount=$RECORDCOUNT -p operationcount=$OPCOUNT -p maxexecutiontime=$RUNTIME -s -t $EXTRAARGS"
