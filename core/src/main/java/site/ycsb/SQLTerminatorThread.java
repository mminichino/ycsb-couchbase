 ...
 *
 */
public class SQLTerminatorThread extends Thread {

  private final Collection<? extends Thread> threads;
  private long maxExecutionTime;
  private SQLWorkload workload;
  private long waitTimeOutInMS;

  public SQLTerminatorThread(long maxExecutionTime, Collection<? extends Thread> threads,
        ...
        try {
          ...
          if (t.isAlive()) {
            System.out.println("Still waiting for thread " + t.getName() + " to complete. " +
                "Workload status: " + workload.workloadRunState());
          }
        } catch (InterruptedException e) {
